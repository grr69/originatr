...
- *Unknown Token*  A token generated by the device.  It's still unknown how to generate it, or what its purpose is. 
...
The purpose of `token` is still unknown.
It is possibly a device ID to uniquely identify the device.
It is a string with 152 characters that appears to match the regex `^[a-zA-Z0-9]{11}:[-_a-zA-Z0-9]{140}$`.
It appears to be generated the first time the app is started, before interacting with the server.
...
POST a request to `https://grindr.mobi/v3/sessions` with this payload:

    {
        "email": "user@example.com",
        "password": "my cool password",
        "token": "<unknown purpose; see discussion above>"
    }
