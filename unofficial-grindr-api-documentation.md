...
Each object points to an array of servers that handles a particular service.  For example, to upload chat photos or user profile photos, you'd use the `upload` service.  These servers don't seem to change very often, so it may be possible to hardcode the server names.
...
The `Session-Id` must be extracted from the HTTP response headers.  The `xmppToken` comes from the json-encoded response payload.  Example response:
...
All authenticated HTTP requests should repeat the `Session-Id` header in the request.  For example, the client might issue:

    PUT /2.0/location HTTP/1.1
    Accept: application/json
    Content-Type: application/json; charset=utf-8
    Session-Id: ABCDEF12341234123412341234012349:01234912309509123490123904102340ABCDEF123401234099124509081234ABBCDEF0129341029309530123094091230490192592309094109235909123490091092349051203940912039409102935
    Content-Length: 58

    {"profileId":"12345678","lon":-123.123456,"lat":45.678901}
        ...
        [
            {
                ...
                "messageId":"12341234-1234-1234-ABCD-ABCDEF012345",
                "sourceClientId":null,
            ...
            {
                "sourceProfileId":"12121212",
                "targetProfileId":"12345678",
                "messageId":"ABCDEF01-1234-1234-ABAB-ABCDEF123456",
                "sourceDisplayName":"\ud83c\udf63",
                "type":"text",
                "timestamp":1454012345678,
                "body":"Hi"
            }
        ]
    }
...
Every 90 seconds, the official client pushes its location to the server.  Issue an authenticated PUT to `https://primus.grindr.com/2.0/location` with the location encoded like this:
