...
<<<<<<< HEAD
var authentication, chat, chatController, cmToLocalUnit, gramToLocalUnit, gui, lastTimeActive, loginController, nativeMenuBar, onEnter, pinpoint, profiles, profilesController, scrollDownOnNewConversation, settingsController, weightInput;
=======
var authenticateFactory, chat, chatController, cmToLocalUnit, directives, fuckr, gramToLocalUnit, gui, kmToLocalUnit, lastTimeActive, managedFields, nativeMenuBar, onEnter, pinpoint, profiles, profilesController, scrollDownOnNewConversation, settingsController, updateLocation, weightInput;
>>>>>>> master
...
authentication = function($localStorage, $http, $rootScope, $q, $location, API_URL) {
  ...
  getGCMToken = function() {
    return $q(function(resolve, reject) {
      if (false && $localStorage.gcmToken) {
  ...
  useCredentials = function(data) {
    ...
    return $rootScope.$emit('authenticated', data.xmppToken);
  };
  return {
    login: function() {
      return $q(function(resolve, reject) {
...
<<<<<<< HEAD
chat = function($http, $localStorage, $rootScope, $q, profiles, API_URL) {
  var acknowledgeMessages, addMessage, client, createConversation, jacasr, nwWindow, s4, sendMessage, uuid;
=======
...
>>>>>>> master
  ...
  return {
    nearby: function(location) {
      ...
      $http.get(API_URL + "locations/" + geohash + "/profiles/online=true").then(function(response) {
        var k, len, profile;
        profiles = _.reject(response.profiles, function(profile) {
    ...
    get: function(id) {
      ...
      if (profileCache[id]) {
        ...
        deferred = $q.defer();
        $http.post(API_URL + 'profiles', {
          targetProfileIds: [id]
        }).then(function(response) {
          return deferred.resolve(response.profiles[0]);
        });
        return deferred.promise;
      }
    },
    ...
    block: function(id) {
      var self;
      self = this;
      return $http.post(API_URL + "blocks/" + id).then(function() {
...
<<<<<<< HEAD
=======
kmToLocalUnit = function($localStorage) {
  return function(km) {
    if (!km) {
      return '';
    } else if ($localStorage.localUnits === 'US') {
      return (km * 0.621371).toPrecision(3) + ' miles';
    } else {
...
<<<<<<< HEAD
fuckr.filter('gramToLocalUnit', ['$localStorage', gramToLocalUnit]).filter('cmToLocalUnit', ['$localStorage', cmToLocalUnit]).filter('lastTimeActive', lastTimeActive).controller('profilesController', ['$scope', '$interval', '$localStorage', '$routeParams', '$window', '$injector', 'profiles', 'pinpoint', profilesController]);
=======
angular.module('profilesController', ['ngRoute', 'ngStorage', 'ngMap', 'profiles', 'pinpoint']).filter('gramToLocalUnit', ['$localStorage', gramToLocalUnit]).filter('cmToLocalUnit', ['$localStorage', cmToLocalUnit]).filter('kmToLocalUnit', ['$localStorage', kmToLocalUnit]).filter('lastTimeActive', lastTimeActive).factory('managedFields', managedFields).controller('profilesController', ['$scope', '$interval', '$localStorage', '$routeParams', '$window', '$injector', 'profiles', 'pinpoint', 'managedFields', profilesController]);
>>>>>>> master
