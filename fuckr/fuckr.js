...
(function() {
  var authenticateFactory, chat, chatController, cmToLocalUnit, fuckr, gramToLocalUnit, gui, highResSrc, lastTimeActive, managedFields, nativeMenuBar, pinpoint, profiles, profilesController, rememberPassword, settingsController, updateLocation, weightInput;
  ...
  managedFields = function($localStorage, $http) {
    var self;
    self = this;
  ...
  angular.module('profilesController', ['ngRoute', 'ngStorage', 'ngMap', 'profiles', 'pinpoint']).directive('highResSrc', highResSrc).filter('gramToLocalUnit', ['$localStorage', gramToLocalUnit]).filter('cmToLocalUnit', ['$localStorage', cmToLocalUnit]).filter('lastTimeActive', lastTimeActive).service('managedFields', managedFields).controller('profilesController', ['$scope', '$interval', '$localStorage', '$routeParams', '$window', '$injector', 'profiles', 'pinpoint', profilesController]);
  ...
  if (typeof process !== 'undefined' && process.versions['node-webkit']) {
    ...
    fuckr = angular.module('fuckr', ['ngRoute', 'profiles', 'profilesController', 'rememberPassword', 'authenticate', 'chat', 'chatController', 'settingsController', 'updateLocation']);
    fuckr.config([
      '$httpProvider', '$routeProvider', function($httpProvider, $routeProvider) {
        var k, len, name, ref, results1, route;
        ...
        for (k = 0, len = ref.length; k < len; k++) {
          route = ref[k];
          name = route.split('/')[1];
          results1.push($routeProvider.when(route, {
  ...
  rememberPassword = function($localStorage) {
    return {
      restrict: 'A',
  ...
  angular.module('rememberPassword', ['ngStorage']).directive('rememberPassword', ['$localStorage', rememberPassword]);
